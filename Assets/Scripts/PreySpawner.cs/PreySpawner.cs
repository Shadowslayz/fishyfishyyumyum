...
public class PreySpawner : MonoBehaviour
{
    ...
    void RefreshTextures()
    {
        ...
        foreach (var file in files)
        {
            ...
            if (textureCache.ContainsKey(texId))
                continue;
            ...
            {
                textureCache[texId] = tex;  // ✅ stored in dictionary
                loadedTextures.Add(tex);
            }

            // Queue new fish for reveal
            if (!knownTextureIds.Contains(texId) && !revealedTracker.IsRevealed(texId))
            {
                orbitQueue.Enqueue(file);
    ...
    private IEnumerator<WaitForSeconds> SpawnAndOrbitNewFish(string texId, string texFile)
    {
        ...
        // ✅ Use cached texture, no disk I/O
        Texture2D tex;
        if (!textureCache.TryGetValue(texId, out tex))
        {
            Debug.LogWarning($"Texture {texId} was not preloaded, loading fallback.");
            byte[] bytes = File.ReadAllBytes(texFile);
        ...
        // ✅ Give Unity 1 frame before orbit to avoid stutter
        yield return null;
    ...
    Vector3 pos = new Vector3(horizontal.x, surfaceY - depth, horizontal.z);
    // Randomize Y rotation
    float randomY = Random.Range(0f, 360f);
    Quaternion rot = Quaternion.Euler(0f, randomY, 0f);
    // If you want to return a position only, just return pos. If you want to apply rotation to a shark GameObject, do it after instantiating.
    // Here, just return the position, but you can use 'rot' when spawning the shark.
    return pos;
    }
